...
def rolling_precisions(window: deque[tuple[list[int], list[int]]]) -> dict[str, float]:
    ...
    return {f"p_{name}": (tp[i] / predicted[i] if predicted[i] else 0.0) for i, name in enumerate(names)}
...
def main() -> int:
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in pbar:
            ...
            pbar.set_postfix(
                loss_1000=sum(loss_window) / len(loss_window),
                acc_1000=sum(acc_window) / len(acc_window),
                **precisions,
            )
